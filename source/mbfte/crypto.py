"""
This module contains all-things-cryptography related to MB-FTE. 
"""
...
BYTES_OF_SENTINEL_DEFAULT: int = 2
"""The default number of bytes for the sentinel."""
BYTES_OF_NONCE_DEFAULT: int = 3
"""The default number of bytes for the nonce."""
BYTES_OF_PADDING_DEFAULT: int = 3
"""The default number of bytes for the padding."""
...
class CiphertextBits:
    ...
    def stats(self) -> Tuple[int, int]:
        """
        Returns the number of ciphertext bits extracted, and the number of extra
        bits extracted.
        """
    ...
    def check_equality(key: bytes, ciphertext: bytes, extra: bytes, n: int) -> bool:
        #_logger.debug("Checking equality for %d bits", n)
        logDebug(f"Checking equality for {n:d} bits")
        if n == 0:
            return True
        xof = CiphertextBits._xof(key, ciphertext)
        nbits: int = 0
        ...
        for byte in extra:
            ...
            #this should format the bytes correctly into a string
            temp = f"Byte: {byte:08b} XOF: {xofbyte:08b}"
            logDebug(temp)
            for offset in range(8):
...
class CryptoSystem(ABC):
    ...
    def __init__(
        self,
        key: bytes,
        bytes_of_sentinel: int = BYTES_OF_SENTINEL_DEFAULT,
        bytes_of_nonce: int = BYTES_OF_NONCE_DEFAULT,
    ) -> None:
        """
...
class RandomPadding(CryptoSystem):
    ...
    def __init__(
        self,
        key: bytes,
        bytes_of_padding: int = BYTES_OF_PADDING_DEFAULT,
        bytes_of_sentinel: int = BYTES_OF_SENTINEL_DEFAULT,
        bytes_of_nonce: int = BYTES_OF_NONCE_DEFAULT,
    ):
