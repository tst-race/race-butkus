...
#from PluginButkus.libPluginButkusEncoding import _logger
...
from PluginButkus.mbfte.crypto import RandomPadding
from PluginButkus.mbfte.model_wrapper import AbstractModelWrapper
from PluginButkus.mbfte.pytorch_model_wrapper import VariableTopKModelWrapper
from PluginButkus.mbfte.utils import (
    #bool_to_symbol,
    find_split_index,
    is_index_valid,
    truncate,
...
class TextCover:
    ...
    def __init__(
        ...
        t1 = time()
        logInfo(f"Time to load model: {(t1 - t0):.4f}s")
        self._seed: str = seed
    ...
    def encode(self, plaintext: Union[str, bytes], complete_sentence: bool = True) -> Tuple[str, int]:
        """
        Encode a plaintext.
        ...
        if isinstance(plaintext, str):
            workText = bytes(plaintext, encoding="utf-8")
        else:
            workText = plaintext

        nonce, ciphertext = self.encrypter.encrypt(workText)
        logDebug(fr"Nonce = {nonce}")
        logDebug(fr"Ciphertext = {ciphertext}")
    ...
    def _fixed_width_adjust(
        ...
        distribution.
        """
        #logDebug(f"cumulative prob:      {cumulative_prob:.4f}")
        #logDebug(f"prev cumulative prob: {prev_cumulative_prob:.4f}")
    ...
    def _encode_fixed_width(
        ...
        while not done:
            ...
            #logDebug(f"# tokens: {len(cumprobs)}")
            while len(cumprobs) == 0:
                logDebug("No tokens to embed into!")
            ...
            #logDebug("High to low? " + bool_to_symbol(high_to_low))
            #logDebug(f"Index:       {i}) [{split_index}]")
            #logDebug("Valid?       " + bool_to_symbol(valid))
            ...
            if valid:
                ...
                #logDebug(f"Low:  {low:08x}")
                #logDebug(f"High: {high:08x}")
                while True:
                    if low[-1] == high[-1]:
            ...
            t1 = time()
            niters += 1
            #change or remove the mod here to alter the amount of iterations printed to log
            if (niters % 100) == 0:
                logInfo(f"Iteration: {niters} {(t1 - t0):.4f}s ({num_encoded_bits} / {max_bits})")


        (nciphertextbits, nextrabits) = ciphertext_bits.stats()
        logInfo(f"# ciphertext bits: {nciphertextbits}")
        logInfo(f"# extra bits: {nextrabits}")

        #build and format the byte string, and then log it
        logDebug("Ciphertext bits: " + "".join(format(byte, '08b') for byte in ciphertext))
        ...
        if self.model.tokenize(covertext) != token_indices:
            logError("Encoding failed: Unable to tokenize produced covertext")
            logDebug("  Covertext: " + covertext)
            #when tokenization fails this leaves token as none
            logDebug(f"  Tokenization: {[token for token in self.model.tokenize(covertext)]}" )
            logDebug(f"  Expected:     {[index for index in token_indices]}" )
            raise UnableToTokenizeCovertext

        logInfo(f"Done encoding! Number of iterations: {niters}")
        logDebug("Covertext: " + covertext)
    ...
    def _decode_fixed_width(
        ...
        for cumprob, prev_cumprob in self._cumprob_gen(covertext):
            ...
            #logDebug(f"Number of bits encoded: {nencoded} / {total_encoded}")

        logDebug("Encoded: " + _bytes_to_bits(_convert_int_to_bytes(encoded, total_encoded)))
        ...
        logError("Decoding failed: Unable to decrypt valid ciphertext")
    ...
    def _cumprob_gen(
        ...
        for i, token_index in enumerate(token_indices, start=1):
            ...
            #logDebug(f"# tokens: {len(cumprobs)}")
            while len(cumprobs) == 0:
                logDebug("No tokens to decode from!")
                cumprobs, state = self.model.prediction(last_token, state, high_to_low)
                logDebug(f"# tokens: {len(cumprobs)}")
            ...
            #change or remove the mod here to alter the amount of iterations printed to log
            if (i % 100) == 0:
                logInfo(f"Iteration {i}: {(t1 - t0):.4f}s")
    ...
    def _try_decrypt(
        ...
        if floor(total_encoded / 8) >= self.encrypter.bytes_to_check():
            ...
            if not self.finish_decryption:
                ...
                logDebug("Bits checked: " + _bytes_to_bits(ct[: self.encrypter.bytes_to_check()]))
                    ...
                    for end in range(self.encrypter.bytes_of_nonce(), len(ct) + 1):
                        ...
                        if plaintext is not None:
                            ...
                            if self.encrypter.check_extra_bits(ct[end:], ct[:end], n):
                                ...
                                logError("Decoding failed: Extra bits not valid")
                                raise ExtraBitsNotValid
                    logWarning(f"No valid ciphertext found up to length {len(ct)} bytes")
                else:
                    logError("Decoding failed: Sentinel check failed")
