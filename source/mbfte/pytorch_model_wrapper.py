...
class PyTorchModelWrapper(AbstractModelWrapper):
    ...
    def __init__(
        ...
        logDebug("Temperature: IGNORED!")
        logDebug("64-bit floats?" + bool_to_symbol(float64))
        if float64:
        ...
        if model_dir == "PRETRAINED":
            ...
            logDebug("Loading pretrained GPT-2 model")
            ...
            logDebug("Loading model from " + model_dir)
            self.tokenizer = transformers.GPT2TokenizerFast.from_pretrained(model_dir)
            self.model = transformers.GPT2LMHeadModel(config)
